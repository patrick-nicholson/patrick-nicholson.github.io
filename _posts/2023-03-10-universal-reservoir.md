...
def approx_nunique(kth_hashes, sample_size):
    """Estimated (only, never exact) distinct elements for one or 
    more kth hashes
    """
    approx = np.where(
        kth_hashes < INT_MAX,
...
sketch_a = set(
    reservoir_a.hashes[reservoir_a.hashes <= combined_kth]
)
sketch_b = set(
    reservoir_b.hashes[reservoir_b.hashes <= combined_kth]
)

intersection_est = len(sketch_a.intersection(sketch_b)) / rate
intersection_exact = len(set_a.intersection(set_b))
difference_est = len(sketch_a.difference(sketch_b)) / rate
